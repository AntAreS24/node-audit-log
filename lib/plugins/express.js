 ...
 *             logBody: true //boolean to log the body of the request or not. This option is ignored for blacklists
 *          }
 */
var ExpressPlugin = function(options) {
    ...
    this.middleware = function(req, res, next) {
        ...
        if (self.logBody(path)) {
            body = req.body;
        }

        self._options.auditLog.logEvent(self._userId, 'express', method, path, body, desc);
    ...
    this.logBody = function(path) {
        ...
        if (self._options.whiteListPaths.length) {
            for (i = 0; i < self._options.whiteListPaths.length; i++) {
                ...
                // If the rule is a regexp, then there is no option, so we can skip to next value.

 ...
 *
 */
var MongooseTransport = function(options) {
    this.name = 'mongoose';
    ...
    this.emit = function(dataObject) {
        var _self = this;
        if (dataObject.object !== null && Object.keys(dataObject.object).length === 0) {
            dataObject.object = ''
        }

        _self.debugMessage('emit: ' + util.inspect(dataObject));

        if (dataObject.logType && dataObject.logType == 'Event') {
            var newEvent = new this.model(dataObject);
            newEvent.save(function(err) {
                if (err) {
                    _self.debugMessage('error saving event to database: ' + err);
                }
            });
        }
    }

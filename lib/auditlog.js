...
function AuditLog() {
     ...
     * Get an already defined transport.
     * 
     * - transport: string name of the transport methodology
     */
    this.getTransport = function(transport) {
        var i;
        for (i = 0; i < this._transports.length; i++) {
            if (this._transports[i].name === transport) {
     ...
     * examples:
     *
     *   actor - date - origin - action - label - object - description
    ...
    // TODO make this an optional call
    //this.socketMonitor = require('./socketmonitor.js');
}
